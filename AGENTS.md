...
│   ├── version.ts                # Package version + published hook command
...
#### CLI (`src/cli.ts`)
- **Commands**: install, uninstall, print, hook, test-event, doctor, help
- **Install options**: --dry-run, --project, --local-command
- **Hook commands**: Published command must come from `getPublishedHookCommand()` in `src/version.ts`; local dev uses `bun src/cli.ts hook`
...
- **Dependency**: `@open-pets/client` (v0.1.1)
...
- **State Mapping**:
  ...
  - `Stop` → `success`
  - `StopFailure` → `error`
- **Auto-Idle**: Terminal states immediately send an idle fallback after success/error so OpenPets can return to idle after temporary animations
...
- Never hardcode `@open-pets/claude-pets@x.y.z` in source files.
- Published hook commands must be generated by `getPublishedHookCommand()` in `src/version.ts`.
- Keep the regression test that compares `getPublishedHookCommand()` to `package.json` version.
- Documentation may use `@<package-version>` placeholders or unpinned `bunx @open-pets/claude-pets` examples, but source-installed hooks should remain pinned to the current package version for reproducibility.
...
- **Runtime**: Bun >= 1.3.0
- **Production**: `@open-pets/client` (0.1.1)
